...
//*****************************************************************************
function paginate($url, $link, $total, $current,$recherche='', $adj=3) {
    ...
    if($recherche!=""){
	$linkRecherche = "SAI_recherche=$recherche"; 
    }
    ...
    if ($total > 1) {
        ...
        if ($total < 7 + ($adj * 2)) {
            ...
            $pagination .= ($current == 1) ? '<span class="active">1</span>' : "<a href=\"{$url}?{$linkRecherche}\">1</a>"; // Opérateur ternaire : (condition) ? 'valeur si vrai' : 'valeur si fausse'
            ...
            for ($i=2; $i<=$total; $i++) {
                if ($i == $current) {
                    ...
                    $pagination .= "<a href=\"{$url}{$link}{$i}&{$linkRecherche}\">{$i}</a>";
                }
            }
        }
        /* =========================================
         ...
         * ========================================= */
            ...
            if ($current < 2 + ($adj * 2)) {
                ...
                $pagination .= ($current == 1) ? "<span class=\"active\">1</span>" : "<a href=\"{$url}?{$linkRecherche}\">1</a>";
                ...
                $pagination .= "<a href=\"{$url}{$link}{$penultimate}&{$linkRecherche}\">{$penultimate}</a>";
                $pagination .= "<a href=\"{$url}{$link}{$total}&{$linkRecherche}\">{$total}</a>";
            }
            /**
            ...
            elseif ( (($adj * 2) + 1 < $current) && ($current < $total - ($adj * 2)) ) {
                ...
                $pagination .= "<a href=\"{$url}?{$linkRecherche}\">1</a>";
                $pagination .= "<a href=\"{$url}{$link}2&{$linkRecherche}\">2</a>";
                $pagination .= '&hellip;';
        ...
        if ($current == $total)
            ...
            $pagination .= "<a href=\"{$url}{$link}{$next}&{$linkRecherche}\">►</a>\n";
